...
import { 
  Star, Share, Heart, MapPin, CheckCircle, Flame, 
  Utensils, MessageSquare, ShieldCheck, ChevronRight 
} from 'lucide-react';
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav className="flex text-sm text-muted-foreground mb-6">
        <ol className="flex items-center space-x-2">
          ...
          <li><Link href="/search" className="hover:text-foreground">Chefs</Link></li>
          <li><ChevronRight className="h-4 w-4" /></li>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-6 gap-4">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 sm:gap-4 text-sm font-medium">
            ...
            <span className="text-muted-foreground underline cursor-pointer">{chef.reviews} reviews</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 grid-rows-2 gap-2 h-[400px] sm:h-[500px] mb-12 rounded-2xl overflow-hidden group">
        <div className="col-span-1 md:col-span-2 row-span-2 relative">
          <img src={chef.coverImage} alt="Main dish" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.02]" />
        </div>
        <div className="hidden md:block col-span-1 row-span-1 relative">
          <img src={chef.gallery[0] || chef.coverImage} alt="Dish 1" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.02]" />
        </div>
        <div className="hidden md:block col-span-1 row-span-1 relative">
          <img src={chef.gallery[1] || chef.coverImage} alt="Dish 2" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.02]" />
        </div>
        <div className="hidden md:block col-span-2 row-span-1 relative">
          <img src={chef.gallery[2] || chef.coverImage} alt="Dish 3" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.02]" />
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        <div className="flex-1 w-full lg:w-2/3">
          ...
          <div className="flex items-center justify-between border-b border-border pb-8">
            <div>
              <h2 className="text-xl sm:text-2xl font-bold mb-1">Prepared by Chef {chef.name.split(' ')[0]}</h2>
              <p className="text-muted-foreground flex gap-2 items-center">
                <span>10+ years experience</span> • <span>100+ events hosted</span>
              </p>
            </div>
            <img 
              src={chef.avatar} 
          ...
          <div className="py-8 border-b border-border">
            ...
            <Button variant="link" className="px-0 mt-2 font-bold text-foreground underline text-base">Show more &rarr;</Button>
          </div>
          ...
          <div className="py-8 border-b border-border">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {chef.specialties.map((spec: string, i: number) => (
                <Badge key={i} variant="secondary" className="px-3 py-1 text-sm">{spec}</Badge>
              ))}
            </div>

            <h3 className="text-lg font-bold mb-4 mt-8">Dietary Capabilities</h3>
            <div className="flex flex-wrap gap-2">
              {chef.dietary.map((diet: string, i: number) => (
                <Badge key={`diet-${i}`} variant="outline" className="px-3 py-1 text-sm bg-background">
          ...
          <div className="py-8">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-2xl font-bold">{chef.rating} <span className="text-muted-foreground font-medium text-xl">· {chef.reviews} reviews</span></h3>
            </div>
          ...
          <div className="sticky top-24 border border-border rounded-2xl p-6 shadow-dropdown bg-card backdrop-blur-xl bg-background/90 z-10 flex flex-col gap-6">
            ...
            <div className="border border-border rounded-xl overflow-hidden divide-y divide-border">
              <div className="grid grid-cols-2 divide-x divide-border">
                <div className="p-3 bg-secondary/30">
                  <label className="text-xs font-bold uppercase tracking-wider text-foreground block mb-1">Date</label>
                  <div className="text-sm font-medium">{date?.toLocaleDateString() || 'Add dates'}</div>
                </div>
                <div className="p-3 bg-secondary/30">
                  <label className="text-xs font-bold uppercase tracking-wider text-foreground block mb-1">Time</label>
                  <div className="text-sm font-medium">19:00 PM</div>
                </div>
              </div>
              <div className="p-3 bg-secondary/30">
                <label className="text-xs font-bold uppercase tracking-wider text-foreground block mb-1">Guests</label>
                <div className="text-sm font-medium">4 Guests</div>
              </div>
            </div>

            <Button size="lg" className="w-full h-14 text-lg bg-accent hover:bg-accent/90 focus-visible:ring-accent" onClick={() => router.push(`/book/${chef.id}`)}>
              Reserve
            </Button>
            ...
            <div className="space-y-4 text-sm mt-4 border-b border-border pb-6">
              <div className="flex justify-between">
                <span className="underline cursor-pointer">${chef.pricePerPerson} x 4 guests</span>
