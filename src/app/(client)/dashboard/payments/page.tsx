      ...
      <div className="grid gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Your primary payment method is used for all bookings.</CardDescription>
          </CardHeader>
            ...
            <div className="flex items-center justify-between p-4 border border-border rounded-xl bg-secondary/30">
              <div className="flex items-center gap-4">
                <div className="h-10 w-12 bg-white rounded border border-border flex items-center justify-center p-1">
                  <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/2/2a/Mastercard-logo.svg/1000px-Mastercard-logo.svg.png" alt="Mastercard" className="h-full object-contain" />
                </div>
                <div>
...
function Badge({ children, variant, className }: any) {
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${variant === 'outline' ? 'border-border text-foreground' : ''} ${className}`}>
      {children}
    </span>
  );
}
