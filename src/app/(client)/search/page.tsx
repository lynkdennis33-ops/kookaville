...
import { Map, List, Search as SearchIcon, SlidersHorizontal, MapPin } from 'lucide-react';
    ...
    <div className="flex flex-col min-h-[calc(100vh-4rem)] pt-6 md:pt-10">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-foreground">Discover Chefs</h1>
            <p className="text-muted-foreground mt-1">24 top-rated chefs available in your area</p>
          </div>

          <div className="flex items-center gap-3">
            <div className="w-full md:w-64 z-10">
              <Select options={sortOptions} value="recommended" onChange={() => {}} />
            </div>
          ...
          <div className={`w-full lg:w-64 flex-shrink-0 ${showFilters ? 'block' : 'hidden lg:block'}`}>
            <div className="sticky top-24 space-y-8 h-[calc(100vh-8rem)] overflow-y-auto pr-4 custom-scrollbar">
              ...
              <div className="space-y-4">
                <h3 className="font-semibold text-lg border-b border-border pb-2">Price Range</h3>
              ...
              <div className="space-y-4">
                <h3 className="font-semibold text-lg border-b border-border pb-2">Cuisine</h3>
                <div className="space-y-3">
                  {categories.slice(0, 5).map((cat) => (
                    <Checkbox key={cat.id} id={`cat-${cat.id}`} label={cat.name} />
                  ))}
                  <button className="text-sm font-medium text-primary hover:text-accent">Show more</button>
                </div>
              </div>

              <div className="space-y-4">
                <h3 className="font-semibold text-lg border-b border-border pb-2">Dietary Needs</h3>
                <div className="space-y-3">
              ...
              <div className="space-y-4">
                <h3 className="font-semibold text-lg border-b border-border pb-2">Rating</h3>
                <div className="space-y-3">
                  {[5, 4, 3].map((rating) => (
                    <Checkbox key={rating} id={`rating-${rating}`} label={`${rating}+ Stars`} />
                  ))}
                </div>
              </div>
              ...
              <div className="h-[600px] w-full rounded-2xl border border-border bg-muted/50 overflow-hidden relative flex flex-col items-center justify-center text-center p-8">
                ...
                <div className="relative z-20 bg-background/80 backdrop-blur-md p-6 rounded-xl border shadow-xl max-w-sm">
                  ...
                  <p className="text-muted-foreground text-sm">
                    In a production environment, this would be integrated with Google Maps or Mapbox API to display precise chef locations and service areas.
                  </p>
                </div>
              </div>
            )}
