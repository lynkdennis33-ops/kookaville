...
import { Users, ChefHat, DollarSign, Activity, TrendingUp, Search, Filter } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Key metrics and detailed tracking.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="border-border">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Verified Chefs</CardTitle>
        ...
        <CardContent className="p-0 flex-1">
          <table className="w-full text-sm text-left">
            <thead className="bg-[#f9fafb] text-[#6b7280] uppercase text-xs">
              <tr>
                <th scope="col" className="px-6 py-4 font-semibold tracking-wider">Booking ID</th>
                <th scope="col" className="px-6 py-4 font-semibold tracking-wider">Client</th>
                <th scope="col" className="px-6 py-4 font-semibold tracking-wider">Chef</th>
            ...
            <tbody className="divide-y divide-border">
              {clientBookings.map((b, i) => (
                <tr key={b.id} className="hover:bg-muted/10 transition-colors">
                  <td className="px-6 py-4 font-mono font-medium text-primary">#{b.id}</td>
                  <td className="px-6 py-4 font-semibold">James Crawford</td>
                  <td className="px-6 py-4 text-muted-foreground">Chef Sarah</td>
                  <td className="px-6 py-4 text-muted-foreground">{new Date(b.date).toLocaleDateString()}</td>
                  <td className="px-6 py-4 font-bold text-emerald-600">${b.totalPrice}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 text-xs font-semibold rounded-full uppercase tracking-wider ${
                      b.status === 'upcoming' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
                    }`}>
                      {b.status}
                    </span>
                  </td>
              ...
              <tr className="hover:bg-muted/10 transition-colors">
                  <td className="px-6 py-4 font-mono font-medium text-primary">#b3</td>
