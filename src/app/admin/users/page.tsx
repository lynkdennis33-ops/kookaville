...
import { User, Shield, Search, MoreVertical, Ban, CheckCircle2 } from 'lucide-react';

export default function AdminUsersPage() {
  const users = [
    { id: 'U1', name: 'James C.', email: 'james@example.com', role: 'Client', joined: 'Mar 1, 2026', status: 'Active' },
    { id: 'U2', name: 'Gordon R.', email: 'gordon@chef.com', role: 'Chef', joined: 'Feb 15, 2026', status: 'Active' },
    { id: 'U3', name: 'Bad Actor', email: 'spam@bot.com', role: 'Client', joined: 'Mar 10, 2026', status: 'Suspended' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex gap-4">
        <Input placeholder="Search users by name or email..." leftIcon={<Search className="h-4 w-4" />} className="max-w-md" />
              ...
              <tbody className="divide-y divide-border">
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-secondary/10 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${user.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                        {user.status}
                      </span>
                    </td>
